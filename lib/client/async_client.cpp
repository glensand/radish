...
public:
    ...
    async_result<T> get(const std::string& key) {
        ...
        get::response resp = co_await detail::async_read<get::response>(*stream_);
        auto [value, ok] = resp.read<T>(*stream_);
        if (!ok) {
...
template<typename T>
async_result<T> async_client::get(const std::string& key) {
    return pimpl->get<T>(key);
}

template<typename T>
async_result<void> async_client::set(const std::string& key, const T& value) {
    return pimpl->set<T>(key, value);
}

async_result<void> async_client::dump() {
    return pimpl->dump();
}
